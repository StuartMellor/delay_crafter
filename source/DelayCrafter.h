...
class DelayCrafter {
    public:
        DelayCrafter(int m_sampleRate, int m_channels) 
            : m_delayTimeMs(500.0f), m_feedbackAmount(0.5f), m_dryWetRatio(0.5f),
              m_channels(m_channels), m_sampleRate(m_sampleRate)
        {
            ...
            m_readPositions.resize(static_cast<size_t>(m_channels), 0);
